...
function startGame(){
  gameStatus =  start;
  document.getElementById("status").innerHTML="Game is loading <span class='spinner-border spinner-border-sm'></span>";
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(keyDown(nosex < 450)){
      character.velocity.x+=gameConfig.moveSpeed;
      character.changeAnimation('move');
      character.mirrorX(1);
    }

    if(!nosex < 350&&!nosex < 450&&!nosey < 200){ 
